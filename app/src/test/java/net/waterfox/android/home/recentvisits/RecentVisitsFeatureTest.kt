...
import net.waterfox.android.home.recentvisits.RecentlyVisitedItemInternal.HistoryGroupInternal
import net.waterfox.android.home.recentvisits.RecentlyVisitedItemInternal.HistoryHighlightInternal
...
class RecentVisitsFeatureTest {
    ...
    fun `GIVEN no recent visits WHEN feature starts THEN fetch history metadata and highlights then notify store`() =
        runTestOnMain {
            ...
            val highlightEntry = HistoryHighlight(1.0, 1, "https://waterfox.net", "waterfox", null)
            val recentHistoryHighlight = RecentHistoryHighlight("waterfox", "https://waterfox.net")
    ...
    fun `GIVEN groups containing history metadata items with the same url WHEN they are added to store THEN entries are deduped`() =
        runTestOnMain {
            ...
            val historyEntry2 = HistoryMetadata(
                key = HistoryMetadataKey("http://waterfox.net", "mozilla", null),
                title = "waterfox",
                createdAt = System.currentTimeMillis(),
                updatedAt = 2,
                totalViewTime = 20,
                documentType = DocumentType.Regular,
                previewImageUrl = "http://waterfox.net/image1"
            )
    ...
    fun `GIVEN different groups containing history metadata items with the same url WHEN they are added to store THEN entries are not deduped`() =
        runTestOnMain {
            ...
            val historyEntry3 = HistoryMetadata(
                key = HistoryMetadataKey("http://www.mozilla.com", "waterfox", null),
                title = "mozilla",
                createdAt = now,
                updatedAt = now + 1,
                totalViewTime = 30,
