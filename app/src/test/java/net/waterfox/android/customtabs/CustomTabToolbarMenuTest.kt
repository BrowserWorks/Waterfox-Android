...
class CustomTabToolbarMenuTest {
    ...
    private lateinit var waterfoxCustomTab: CustomTabSessionState
    ...
    fun setUp() {
        ...
        waterfoxCustomTab = createCustomTab(url = "https://waterfox.net", id = "123")
        ...
        customTabToolbarMenu = spyk(
            CustomTabToolbarMenu(
                context = context,
                store = store,
                sessionId = waterfoxCustomTab.id,
    ...
    fun `custom tab toolbar menu uses the proper custom tab session`() {
        assertEquals(waterfoxCustomTab.id, customTabToolbarMenu.session?.id)
        assertEquals("https://waterfox.net", customTabToolbarMenu.session?.content?.url)
    }
}
