...
/**
 * Waterfox implementation of [FxAEntryPoint].
 */
enum class WaterfoxFxAEntryPoint(override val entryName: String) : FxAEntryPoint, Parcelable {
    /**
     ...
     */
    companion object CREATOR : Parcelable.Creator<WaterfoxFxAEntryPoint> {
        override fun createFromParcel(parcel: Parcel): WaterfoxFxAEntryPoint {
            val parcelEntryName = parcel.readString() ?: Unknown
            return WaterfoxFxAEntryPoint.values().first { it.entryName == parcelEntryName }
        }

        override fun newArray(size: Int): Array<WaterfoxFxAEntryPoint?> {
