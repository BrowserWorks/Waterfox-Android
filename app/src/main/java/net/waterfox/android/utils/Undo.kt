 ...
 * @param message A message displayed as part of [WaterfoxSnackbar].
 * @param undoActionTitle Label for the action associated with the [WaterfoxSnackbar].
 * @param onCancel A suspend block to execute in case of cancellation.
 * @param operation A suspend block to execute if user doesn't cancel via the displayed [WaterfoxSnackbar].
 * @param anchorView A [View] to which [WaterfoxSnackbar] should be anchored.
 */
...
fun CoroutineScope.allowUndo(
    ...
    fun showUndoSnackbar() {
        val snackbar = WaterfoxSnackbar
            .make(
                view = view,
                duration = WaterfoxSnackbar.LENGTH_INDEFINITE,
